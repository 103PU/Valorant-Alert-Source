...
function getRegionInfo() {
  ...
  if (!fs.existsSync(logPath)) {
    logger.warn('ShooterGame.log not found, using default region "ap".');
    return getDefaultRegionInfo();
  }
  ...
  try {
    ...
    logger.info(`Detected Region: ${region}, Shard: ${shard}, ClientVersion: ${cachedRegionData.clientVersion}`);
    return cachedRegionData;
  } catch (err) {
    logger.error('Error reading ShooterGame.log:', err.message);
    return getDefaultRegionInfo();
  }
}
