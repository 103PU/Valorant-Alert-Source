  ...
  init() {
    ...
    this.httpServer.on('upgrade', (request, socket, head) => {
      ...
      if (!clientToken || clientToken !== this.authToken) {
        console.warn(`[WSServer] Handshake rejected. Invalid token: "${clientToken}"`);
    ...
    this.wss.on('connection', (ws, req) => {
      ...
      console.log(`[WSServer] iPhone Client authenticated and connected from: ${clientIp}`);
      ...
      ws.on('close', () => {
        console.log(`[WSServer] Client disconnected: ${clientIp}`);
      });
    });
  }
